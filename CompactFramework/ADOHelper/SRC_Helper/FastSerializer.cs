    ...
    public sealed class SerializationWriter : BinaryWriter
    {
        ...
        public void WriteObject(object value)
        {
            ...
            else if (value is Decimal)
            {
                ...
                else
                {
                    ...
                    int[] IntBuff = Decimal.GetBits(decimalValue);
                    byte[] ByteBuff = new byte[16];
                    Buffer.BlockCopy(IntBuff, 0, ByteBuff, 0, 16);
                    Write(ByteBuff);
                    //Write(decimalValue); ///!!!!!!!!!!!!!!!!!
                }
            }
            ...
            {
                throw new Exception("Serialize: the execution should not come to this point:" + value.GetType().ToString());
        ...
        private static bool isTypeRecreatable(Type type)
        {
            ...
            else
            {
                Type[] types = new Type[] { };
                // !!!!!!!!!!!!!!!
                //return typeof(IOwnedDataSerializableAndRecreatable).IsAssignableFrom(type) && type.GetConstructor(Type.EmptyTypes) != null; // orig
                return typeof(IOwnedDataSerializableAndRecreatable).IsAssignableFrom(type) && type.GetConstructor(types) != null;
            }
        }
    ...
    public sealed class SerializationReader : BinaryReader
    {
        ...
        private object processObject(SerializedType typeCode)
        {
            ...
            else if (typeCode == SerializedType.DecimalType)
            {
                ...
                byte[] ByteBuff = new byte[16];
                Read(ByteBuff, 0, 16);
                Buffer.BlockCopy(ByteBuff, 0, array, 0, 16);
            ...
            {
                throw new Exception("Deserialize: the execution should not come to this point: no surrogate was set" );
